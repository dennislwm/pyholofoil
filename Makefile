...
help:
	...
	@echo "  build      Materialize data/products_public.db, redacted per sensitive_fields.yaml (ADR-04, ADR-27)"
	@echo "  deploy     Verify REDACTED_DB_PATH excludes every sensitive_fields.yaml column (REQ-013), then copy it into DOCS_DIR (default docs/) for CI to upload as a GitHub Pages artifact (ADR-17) -- view it via datasette-lite (ADR-16): https://lite.datasette.io/?url=https://$(GH_PAGES_HOST)/products_public.db"
