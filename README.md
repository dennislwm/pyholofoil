...
4. **`make build`** -- materializes `data/products_public.db`, a redacted copy with `sensitive_fields.json`'s columns stripped. Requires [sensitive fields populated](#setup-redacting-sensitive-fields) -- an empty list ships everything unredacted.
5. **`make deploy`**:
